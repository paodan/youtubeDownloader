...
#' @description Download YouTube videos/audios by providing an URL seed, which is any
#' URL in a video list.
...
videoListDownload = function(urlSeed, path, saveFileList = T, sleepTime = 10, maxDownload = 1000,
                             priority = c("audio only","best","mp4"), bothVideoAudio = T, webm2mp3 = F){
  # copyright: Weiyang Tao 2017-11-02
  urlpage = readLines(urlSeed, warn = F)
  ##### File names with orders
  # the line containing titles
  titleLine = grep("data-video-title=", urlpage, perl = F, value = T)
  ...
  tmp = sapply(1:num, function(ti) make.names(paste0(nameID[ti], titles[ti, 1], collapse = ""), unique = F, allow_ = T))
  orderTitle = data.frame(Orders = orders, VideoTitles = titles, fileName = tmp, stringsAsFactors = F)
  # the line containing ture url
  id = grep("<a href=\"\\/watch\\?v=.*&amp;index", urlpage, perl = F, value = T)
  if (length(id) == 0) {
    ...
    orderTitle$URL = paste0('https://www.youtube.com/watch?v=', v,
                            "&list=", list0, "&index=", index) # generate url
    foldID = grep(paste0("\\/playlist\\?list=", list0, "\" class=\" yt-uix-sessionlink      spf-link "), urlpage, perl = T, value = T)
    folderName = substr(foldID, regexpr("\" >", foldID) + 3, regexpr("<\\/a>", foldID)-1)
    folderName = file.path(path, make.names(folderName))
    print(folderName)
    if (!dir.exists(folderName)) dir.create(folderName)
    # save file name and URLs
    if (saveFileList) write.csv(orderTitle, paste0(folderName, "/fileNameOrders.csv"))
    # download videos
    ...
    for(ui in 1:num){
      ...
      youTubeDownload(url = url_ui, path = folderName, saveFile = file_ui,
                      priority = priority, bothVideoAudio = bothVideoAudio)
